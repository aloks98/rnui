/**
 * Minimal preset — applies sensible defaults without overwriting user styling.
 *
...
function mergeDefaults<T>(value: T, defaults: any): T {
  if (value === undefined) return defaults as T
...
function formatTooltipValue(value: unknown): string {
  ...
  if (isPlainObject(value)) {
    ...
    if (maybe !== undefined) return formatTooltipValue(maybe)
  }
...
function hasTooltipFormatting(tooltip: unknown): boolean {
  if (!isPlainObject(tooltip)) return false
...
function hasSeriesTooltipFormatting(series: unknown): boolean {
  ...
  return list.some(
    (entry) => isPlainObject(entry) && hasTooltipFormatting(entry.tooltip),
  )
}
