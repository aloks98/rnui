 ...
 */
export function extractShadcnColors(element?: HTMLElement): ShadcnChartColors {
  return {
...
export function buildEChartsTheme(
  ...
  const mutedTextColor = resolveColor(
    ...
    {
      fallback:
        mode === 'dark' ? 'rgba(244, 244, 245, 0.70)' : 'rgba(9, 9, 11, 0.60)',
    },
  )
  ...
  const mutedColor = resolveColor(colors.muted ?? '', {
    fallback:
      mode === 'dark' ? 'rgba(244, 244, 245, 0.08)' : 'rgba(9, 9, 11, 0.04)',
  })
